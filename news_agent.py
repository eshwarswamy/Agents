...
# ── Customize these RSS feeds as you like ──────────────────────────────────────
RSS_FEEDS = {
...
# ── Deduplication — skip if digest identical to yesterday ─────────────────────

def digest_hash(articles: list[dict]) -> str:
      blob = json.dumps([a["title"] for a in articles], sort_keys=True)
...
# ── Error Alert (optional self-notification on failure) ────────────────────────

def alert_failure(error_msg: str) -> None:
      """Sends a short Telegram alert if main pipeline fails."""
      try:
          send_telegram(f"⚠️ *News Agent Failed*\n```\n{error_msg[:300]}\n```")
      except Exception:
          pass
